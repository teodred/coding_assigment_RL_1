...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
	...
	d = tol+1
	while d > tol :
		value_function_updated = np.zeros(nS)
		for s in range(nS):
			a = policy[s]
			...
			for p, s_, r, t in P[s][a]:
				if not t:
					s_ = int(s_)
					value_function_updated[s] += r+gamma*p*value_function[s_] 
				else:
					value_function_updated[s] += r
		d = max(abs(value_function-value_function_updated))

		value_function = value_function_updated
	############################
	...
	for s in range(nS):
		...
		for a in range(nA):
			for p, s_, r, t in P[s][a]:
				if not t:
					s_ = int(s_)
					q_a[a] += r+gamma*p*value_from_policy[s_]
				else:
					q_a[a]+=r
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
	...
	while i==0 or d > 0 :
		...
		policy_improve = policy_improvement(P, nS, nA, value_from_policy, policy, gamma)
		i+=1
			...
			for a in range(nA): 
				for p, s_, r, t in P[s][a]:
					if not t:
						...
						op_a[a] += r
			value_function_updated[s] = np.max(op_a)
			policy[s] = np.argmax(op_a)
		d=max(abs(value_function_updated-value_function))
		value_function = value_function_updated
